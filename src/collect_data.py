...
# 필요시 .env의 DTL_PRDCT_NOS(콤마 구분)로 덮어쓸 수 있습니다. 이 API는 세부품명번호
# 없이는 조회가 안 되므로 비워둘 수 없습니다(fetch_rows에서 에러 발생).
# xlsx 원본 대조로 경쟁사가 실제 거래하는 업종을 확인해 추가한 코드 목록:
#   4323300101 시스템관리소프트웨어, 8111159801 패키지소프트웨어개발및도입서비스,
#   4323290201 통신소프트웨어, 8111229901 소프트웨어유지및지원서비스,
#   8111151318 성능관리클라우드서비스(SaaS)
DTL_PRDCT_NOS = [
    s.strip()
    for s in os.getenv(
        "DTL_PRDCT_NOS",
        "4323300101,8111159801,4323290201,8111229901,8111151318",
    ).split(",")
    if s.strip()
]
